...
export default function MotorIAPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] w-full overflow-hidden animate-fade-in-up bg-[#080908]">
      
      {/* HEADER: LENTES HERMENÉUTICOS */}
      <div className="flex-shrink-0 px-8 py-5 border-b border-[#D1A65B]/10 bg-[#0c0d0c] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-20 shadow-md">
        <div>
          <h1 className="text-2xl font-serif text-[#F9F6F0] flex items-center gap-3">
            <BrainCircuit className="text-[#c4b5fd]" size={28} /> Motor de Inferencia RAG
          </h1>
          <p className="text-[#F9F6F0]/50 text-xs mt-1">Selecciona el lente teológico antes de consultar.</p>
        </div>

        {/* SELECTOR DE LENTES (AHORA FUNCIONAN) */}
        <div className="flex bg-[#151715] p-1.5 rounded-xl border border-[#D1A65B]/20 shadow-inner overflow-x-auto w-full md:w-auto scrollbar-hide">
          {['Raíces Originales', 'Contexto Histórico', 'Teología Sistemática'].map((lente) => (
            ...
            >
              {lenteSeleccionado === lente && <Sparkles size={14} />} {lente}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col border-r border-[#D1A65B]/10 relative bg-[#0c0d0c]/50">
          
          <div className="flex-1 overflow-y-auto p-8 flex flex-col justify-center items-center">
            
            {/* Si no hay respuesta ni está cargando, mostramos el Analizador de Tesis */}
            {!respuestaIA && !cargando && (
              <div className="max-w-xl w-full">
                <div className="relative group cursor-pointer">
                  <div className="absolute -inset-1 bg-gradient-to-r from-[#D1A65B] to-[#7c3aed] rounded-3xl blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200 animate-pulse"></div>
                  <div className="relative bg-[#080908] border-2 border-dashed border-[#D1A65B]/30 hover:border-[#7c3aed]/60 rounded-2xl p-10 flex flex-col items-center justify-center text-center transition-all shadow-2xl">
                    <div className="w-20 h-20 bg-[#151715] rounded-full border border-[#7c3aed]/30 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(124,58,237,0.1)] group-hover:scale-110 transition-transform">
                      <UploadCloud size={32} className="text-[#c4b5fd]" />
                    </div>
                    <h3 className="text-2xl font-serif text-[#F9F6F0] mb-3">Analizador Teológico</h3>
                    <p className="text-[#F9F6F0]/60 text-sm font-light mb-6 max-w-sm">
            ...
            {/* Si está cargando, mostramos una animación */}
            {cargando && (
              <div className="flex flex-col items-center justify-center text-[#c4b5fd] animate-pulse">
                <Loader2 size={48} className="animate-spin mb-4" />
                <p className="font-serif text-lg">Consultando los pergaminos digitales...</p>
              </div>
            )}

            {/* Si ya hay respuesta, la mostramos como un documento elegante */}
            {respuestaIA && !cargando && (
              <div className="w-full max-w-3xl bg-[#151715]/80 border border-[#7c3aed]/30 p-8 rounded-2xl shadow-2xl relative animate-fade-in-up">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#7c3aed]/10 blur-[50px] rounded-full"></div>
                <h3 className="text-[#c4b5fd] text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                  <BrainCircuit size={16} /> Respuesta del Erudito IA
                </h3>
                <div className="prose prose-invert prose-p:text-[#F9F6F0]/80 prose-p:font-light prose-p:leading-relaxed max-w-none whitespace-pre-wrap">
                  {respuestaIA}
                </div>
              </div>
            )}
          ...
          {/* INPUT DE CHAT (AHORA CONECTADO) */}
          <div className="p-6 bg-[#0c0d0c] border-t border-[#D1A65B]/10">
            <div className="max-w-4xl mx-auto bg-[#151715] border border-[#7c3aed]/30 rounded-2xl p-2 pl-6 flex items-center gap-4 focus-within:border-[#c4b5fd]/60 focus-within:shadow-[0_0_20px_rgba(124,58,237,0.15)] transition-all">
              <Sparkles size={20} className="text-[#c4b5fd] flex-shrink-0" />
              <input 
                type="text" 
                value={pregunta}
                onChange={(e) => setPregunta(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && consultarGemini()}
                placeholder="Ejemplo: ¿Cuál es el significado de 'Logos' en Juan 1:1?" 
                className="w-full bg-transparent border-none text-[#F9F6F0] placeholder-[#F9F6F0]/30 focus:outline-none focus:ring-0 text-base py-3"
                disabled={cargando}
              />
              <button 
                onClick={consultarGemini}
                disabled={cargando || !pregunta.trim()}
                className="bg-gradient-to-r from-[#7c3aed] to-[#5b21b6] hover:from-[#8b5cf6] hover:to-[#6d28d9] text-[#F9F6F0] p-3 rounded-xl transition-all shadow-lg flex-shrink-0 disabled:opacity-50"
              >
                <Send size={20} />
              </button>
            </div>
            <p className="text-center text-[#F9F6F0]/30 text-[10px] mt-3">El Motor IA puede cometer errores. Verifica siempre con el texto bíblico original.</p>
          </div>
        </div>

        {/* === LADO DERECHO: CONSTELACIÓN Y CÓDICE === */}
        <div className="w-full lg:w-[450px] bg-[#080908] flex flex-col h-full border-l border-[#D1A65B]/10">
